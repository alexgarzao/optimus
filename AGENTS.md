...
This repository contains installable skill plugins for Droid (Factory), Claude Code,
and OpenCode (sst/opencode). Every change to this repo is made by AI agents. This
file is the primary context source for any agent operating here.
...
Optimus skills can be installed on three platforms simultaneously, each using its
native distribution mechanism:

| Aspect | Droid (Factory) | Claude Code | OpenCode |
|--------|-----------------|-------------|----------|
| **Install mechanism** | `droid plugin install/update/uninstall` | `claude plugin install/update/uninstall` | Filesystem symlinks under `~/.config/opencode/` (no plugin CLI) |
| **Manifest** | `.factory-plugin/marketplace.json` (17 bare-name plugins) | `.claude-plugin/marketplace.json` (1 plugin: `optimus`) | `.opencode-plugin/plugin.json` (version marker only) |
| **Skill discovery** | One plugin per skill, registered individually | Single `optimus` plugin sourced from repo root; commands discovered from top-level `commands/` | Symlinked SKILL.md under `~/.config/opencode/skills/optimus-<plugin>/` |
| **Plugin name format** | Bare per-skill (`plan`, `build`, ...) | Single `optimus` plugin | Per-skill symlinks (`optimus-plan`, ...) |
| **Invocation** | `/<plugin>` (e.g., `/optimus-plan`) or alias `/sp` | `/optimus:<command>` (e.g., `/optimus:plan`) | `/optimus-<command>` (e.g., `/optimus-plan`) — bare prefixed |
| **Command aliases** | Supported as bare slash commands (`/sp`, `/bd`) | Supported as namespaced (`/optimus:sp`, `/optimus:bd`) | Supported prefixed (`/optimus-sp`, `/optimus-bd`) |

The Droid model preserves a separate plugin per skill because Droid's CLI does
not implement the `/<plugin>:<command>` namespace syntax (commands are invoked
as `/<command-name>` only — see https://docs.factory.ai/cli/configuration/plugins).
The Claude model collapses to one plugin so the user sees a clean
`/optimus:<command>` namespace in autocomplete instead of 17 separate entries.
The OpenCode model has no marketplace at all in the documented behavior, so we
use filesystem symlinks back to the repo source — meaning `git pull` upstream
propagates instantly without re-syncing.

The top-level `commands/<plugin>.md` and `commands/<alias>.md` files are
generated by `scripts/sync-claude-commands.py` from each `<plugin>/skills/optimus-<plugin>/SKILL.md`
(and `<plugin>/commands/<alias>.md`). The OpenCode equivalents live at
`commands/opencode/optimus-<cmd>.md` and are generated by
`scripts/sync-opencode-commands.py`. Both generators share parsing logic
via `scripts/_sync_commands_common.py`. Run `make sync-commands` after editing
a SKILL.md or alias body to keep both targets in sync. The
`TestClaudeCommandsSync` and `TestOpenCodeCommandsSync` classes in
`scripts/test_skill_consistency.py` enforce that both scripts were run before commit.

The sync script (`sync/scripts/sync-user-plugins.sh`) detects which platforms
are available and syncs all in a single run via each platform's native mechanism.

Platform detection:
- **Droid**: `command -v droid` succeeds
- **Claude Code**: `command -v claude` succeeds
- **OpenCode**: `command -v opencode` succeeds

At least one platform must be available. The Droid and Claude branches use
each platform's marketplace cache as the source of truth for expected
plugins; the OpenCode branch uses the repo itself (no marketplace exists).

See `docs/opencode-platform-support.md` for OpenCode-specific notes (no
marketplace, bare-name invocation, no iTerm2 hook, conflict handling).

### Marketplace JSON resolution

The script resolves the EXPECTED plugin list (used by the Droid branch only)
...
Only the `.factory-plugin/marketplace.json` source enumerates per-skill bare
names. The Claude Code marketplace lists a single `optimus` plugin, so source 2
is only used as a fallback when Droid is missing AND the in-repo Droid file is
not available — in that case the EXPECTED list collapses to `["optimus"]` and
the Droid branch is skipped (Droid is also unavailable in that scenario).

### Claude Code specifics

- A single `optimus@optimus` plugin is installed via `claude plugin install optimus@optimus --scope user`
- The plugin source is the repo root; the `commands/` directory at root holds the slash-command files
- Updates go through `claude plugin update optimus@optimus`
- Claude Code manages its own plugin cache at `~/.claude/plugins/cache/optimus/optimus/<version>/`;
  the script does not create or manage `~/.optimus/repo/` for Claude Code
- Pre-2.0 installations had 17 separate `optimus-<name>@optimus` plugins. The
  sync script silently uninstalls any of those it finds so users migrate
  cleanly to the single-plugin model.

### Environment variables

| Variable | Default | Purpose |
|----------|---------|---------|
| `OPTIMUS_REPO_URL` | `https://github.com/alexgarzao/optimus` | Repo URL (used for non-default forks; all platforms honor) |
...
| `OPENCODE_CONFIG_DIR` | `${XDG_CONFIG_HOME:-$HOME/.config}/opencode` | Override OpenCode config root for symlink target paths |

## Plugin Lifecycle

### Dual-platform parity (Claude Code ↔ Droid) — ENFORCED

Every Optimus skill MUST ship surfaces for both platforms. This is enforced by
the `TestDualPlatformParity` and `TestClaudeCommandsSync` classes in
`scripts/test_skill_consistency.py`, which assert four invariants:

1. **Single Claude plugin** — `.claude-plugin/marketplace.json` lists exactly
   one plugin named `optimus`, sourced from `.` (repo root). The 17 commands
   surface as `/optimus:<command>` from the top-level `commands/` directory.
2. **Surface parity** — every plugin in `.factory-plugin/marketplace.json`
   must ship: `<plugin>/skills/optimus-<plugin>/SKILL.md`, `<plugin>/.factory-plugin/plugin.json`
   (Droid), AND `commands/<plugin>.md` (Claude command, generated from the
   SKILL.md by `scripts/sync-claude-commands.py`).
3. **Command alias integrity** — every `<plugin>/commands/<alias>.md`
   corresponds to a real plugin in the Droid marketplace AND its body
   redirects to `/optimus-<plugin>`. The flattened `commands/<alias>.md`
   files at repo root rewrite that redirect to `/optimus:<plugin>`.
4. **Claude commands in sync** — the body of every `commands/<plugin>.md`
   matches the body of `<plugin>/skills/optimus-<plugin>/SKILL.md` after
   applying the `/optimus-X` -> `/optimus:X` substitution. If they
   disagree, the contributor forgot to run `scripts/sync-claude-commands.py`.

A skill that only ships one platform's surface — or whose top-level Claude
command drifted from its SKILL.md — fails tests and CI. Without these guards,
a contributor could add a skill to the Droid marketplace, forget to regenerate
the Claude commands, and Claude users would never see it (because the single
`optimus` plugin discovers commands from the on-disk `commands/` directory,
not from a per-skill marketplace entry).

### Adding a new skill
1. Add directory to repo (e.g., `myplugin/skills/optimus-myplugin/SKILL.md`)
2. Add `<myplugin>/.factory-plugin/plugin.json`
3. Add entry to `.factory-plugin/marketplace.json` (bare name: `myplugin`)
4. (Optional) Add `<myplugin>/commands/<alias>.md` if the skill has a short alias
5. Run `python3 scripts/sync-claude-commands.py` to regenerate the top-level
   `commands/<myplugin>.md` (and any alias) for the Claude side
6. Update README plugin list if applicable
7. Commit, push
8. **Droid**: `droid plugin install myplugin@optimus` (or run `/optimus-sync`)
9. **Claude Code**: re-running `/optimus-sync` updates the single `optimus@optimus` plugin so the new command appears at `/optimus:myplugin` (and `/optimus:<alias>` if any). No new Claude marketplace entry is needed.

### Updating a skill
1. Edit the SKILL.md
2. Run `python3 scripts/sync-claude-commands.py` to regenerate the matching `commands/<plugin>.md`
3. Commit, push
4. Run `/optimus-sync` (or `make sync-plugins`) — script handles both platforms

### Removing a skill
1. Remove the per-skill directory
2. Remove the entry from `.factory-plugin/marketplace.json` (the Claude marketplace stays unchanged — it has only the single `optimus` entry)
3. Run `python3 scripts/sync-claude-commands.py` to prune the orphaned `commands/<plugin>.md` and any aliases
4. Update README if applicable
5. Commit, push
6. Run `/optimus-sync` — the Droid branch removes the orphan plugin; the Claude branch updates the single `optimus@optimus` plugin so the removed command disappears from the on-disk `commands/`.

**Note for end users:** Claude Code plugins do NOT auto-update. Users must run
`/optimus-sync` to pick up new and removed commands. The
